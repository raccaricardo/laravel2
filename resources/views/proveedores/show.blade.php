    ...
    @else
        ...
        <form action="{{ url('/providers/' . $item->id) }}" id='form' method='post'>
            ...
            <div class="form-group mt-2">
                ...
                <input disabled value="{{ $item->business_name }}" type="text" class="form-control"
                    id="input_business_name" name="input_business_name" aria-describedby="surname"
                    placeholder="Ingrese razon social">
            </div>
            <div class="form-group mt-2">
                <label for="input_address">Direccion</label>
                <input disabled value="{{ $item->address }}" type="text" class="form-control" id="input_address"
                    name="input_address" aria-describedby="address" placeholder="Ingrese direccion">
            </div>
            <div class="form-group mt-2">
                <label for="input_phone">CP</label>
                <input disabled value="{{ $item->cp }}" type="text" class="form-control" id="input_cp"
                    name="input_cp" aria-describedby="phone" placeholder="Ingrese codigo postal">
            </div>
            <div class="form-group mt-2">
                <div class="row">
                        ...
                        @if (isset($ivas))
                            @foreach ($ivas as $iva)
                                @if ($iva->id == $item->fiscalCondition->id)
                                    <option value="{{ $item->fiscalCondition->id }} selected">
                                        {{ $item->fiscalCondition->name }}</option>
                                @else
            ...
            @if (isset($editable))
                <div class="form-group mt-2">
                    ...
                    <input disabled value="{{ $item->email }}" type="email" class="form-control" id="input_email"
                        name="input_email" aria-describedby="emailHelp" placeholder="Ingrese correo electronico">
                </div>
                <div class="form-group mt-2">
                    <label for="input_website">Sitio web</label>
                    <input disabled value="{{ $item->website }}" type="url" class="form-control" id="input_website"
                        name="input_website" placeholder="Ingrese sitio web del proveedor">
                </div>
            @endif
